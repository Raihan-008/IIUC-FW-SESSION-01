...
residency = input("Are you a resident of the country? (yes/no): ").strip().lower()
...
def categorize_day(temperature, weather_condition):
    ...
    if temperature > 25:  
        if weather_condition == "sunny" or weather_condition == "cloudy":
            return "Beach Day"
        else:
            return "Stay Home Day"  

    elif temperature < 0:  
        if weather_condition == "snowy" or weather_condition == "cloudy":
            return "Skiing Day"
        else:
            return "Stay Home Day"  
...
weather_condition = input("Enter the weather condition (sunny/cloudy/rainy/snowy/stormy): ").strip().lower()

day_category = categorize_day(temperature, weather_condition)


print(f"The day is categorized as: {day_category}")
