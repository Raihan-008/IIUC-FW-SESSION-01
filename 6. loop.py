...
temperature_readings = [22.5, 30.1, 18.3, 35.2, 27.8, 40.5, 24.9]
threshold = 30.0

for temperature in temperature_readings:
    if temperature > threshold:
        print(f"Warning: Temperature {temperature}°C is above the threshold!")
...
for device_name, details in products.items():
    stock =details["stock"]
    min_requirement = details["min_required"]
    
    if(stock < min_requirement):
        print(f"Alert: {device_name.capitalize()} stock is low. Current stock: {stock}. Minimum required: {min_requirement}.")
    else:
        print(f"{device_name.capitalize()} has sufficient stock. Current stock: {stock}. Minimum required: {min_requirement}.")
